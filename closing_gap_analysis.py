...
def load_all_history():
    ...
    for fname in sorted(os.listdir(HIST_DIR)):
        if fname.endswith(".jsonl"):
            all_history.extend(
                load_jsonl(os.path.join(HIST_DIR, fname))
            )
...
def compute_live(history):
    ...
    sorted_hist = sorted(
        history,
        key=lambda h: parse_iso(h.get("ts"))
    ...
    for asset_name, asset_data in latest.get("assets", {}).items():
        ...
        if asset_name not in series_by_asset:
            series_by_asset[asset_name] = get_asset_series(
                history,
                asset_name
            )
        ...
        for market in asset_data.get("markets", []):
            ...
            if current_price > strike:
                ...
                explanation = (
                    f"{asset_name} "
                    f"${current_price - strike:.2f} above strike"
                )
                ...
                explanation = (
                    f"{asset_name} "
                    f"${strike - current_price:.2f} below strike"
                )
            ...
            if velocity and velocity > 0:
                seconds_to_cross_val = round(
                    abs(current_price - strike) / velocity,
                    1
                )
...
def main():
    ...
    print(
        f"Closing-gap analysis ({mode}): "
        f"{len(live_calls)} live calls in final "

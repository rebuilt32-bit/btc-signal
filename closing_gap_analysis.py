...
Two outputs:
  - LIVE: today's currently-open markets in their final 3 minutes
  - BACKTEST: historical accuracy of margin-ratio buckets

Strategy: in the final stretch, gap-vs-velocity is a near-mechanical call.
Margin ratio = seconds_to_cross / seconds_left.
A 2x+ margin ratio means the price would need 2x more time than remains
to cross the strike — so the currently-winning side is very likely to stay winning.

Output: data/closing_gap_analysis.json
"""
import json
import os
...
OUT_PATH = "data/closing_gap_analysis.json"
...
def compute_live(history):
    """Live margin-ratio calls for currently-open markets in final 3 minutes."""
    if not history:
        return []
    sorted_hist = sorted(
        history,
...
def compute_backtest(history):
    """For each settled ticker, evaluate margin-ratio call at each checkpoint."""
    ...
    for b in BUCKETS:
        label = b["label"]
        stats = by_bucket.get(label, {"n": 0, "wins": 0, "n_uncalled": 0})
        n = stats["n"]
        bucket_summary[label] = {
            "n_calls": n,
            "n_wins": stats["wins"],
            "win_rate": round(stats["wins"] / n, 3) if n > 0 else None,
            "n_at_strike_skipped": stats["n_uncalled"],
        }
    ...
    for b in BUCKETS:
        ...
        for cp_def in BACKTEST_CHECKPOINTS:
            ...
            stats = by_bucket_x_cp.get(label, {}).get(cp_label, {"n": 0, "wins": 0})
            n = stats["n"]
            bucket_x_cp[label][cp_label] = {
                "n": n,
                "win_rate": round(stats["wins"] / n, 3) if n > 0 else None,
            }
    ...
    return {
        ...
        "by_bucket_x_checkpoint": bucket_x_cp,
    }
...
def main():
        ...
        "config": {
            ...
            "buckets": [
                {"label": b["label"], "min": b["min"],
                 "max": b["max"] if b["max"] != float("inf") else "inf"}
                for b in BUCKETS
            ],
        },
        "interpretation": (
    ...
    print(f"Closing-gap analysis: {len(live_calls)} live calls in final {WINDOW_SECONDS}s window")

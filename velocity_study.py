"""Velocity formula study — compare endpoint, stdev, path-length velocity calcs."""
...
OUT_PATH = "data/velocity_study.json"
...
def main():
    ...
    if not samples:
        print("No samples — likely need more 1-sec-cadence data.")
        return
    ...
    print(f"\nWrote {OUT_PATH}")
    print("\nLook at the 'extreme' row for each formula. The one with highest win rate")
    print("AND most n is the best velocity formula. If stdev or pathlen beats endpoint,")
    print("we should switch closing_gap_analysis.py to use it.")
